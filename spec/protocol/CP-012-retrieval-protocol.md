...
> **Author**: Nadeem Bhati
> **Category**: Protocol
> **Requires**: CP-001, CP-002, CP-010, CP-003

## Synopsis

This specification defines the hybrid retrieval protocol for CP, combining semantic (vector) search with lexical (keyword) search using Reciprocal Rank Fusion (RRF).
...
When scores are equal, CP uses deterministic tiebreakers:
...
CP supports multiple query modes:
...
CP supports pre-retrieval and post-retrieval filtering.
...
CP uses **Integer Dot Product** for similarity search, which is equivalent to Cosine Similarity for normalized vectors but deterministic.
