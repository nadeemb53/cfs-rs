...
This specification defines the hybrid storage architecture used by CP, combining SQLite for structured data persistence with HNSW (Hierarchical Navigable Small World) graphs for vector similarity search.

## Motivation

CP requires a storage engine that provides:
...
CP uses versioned migrations for schema changes:
