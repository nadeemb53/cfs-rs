            ...
            let result: (answer: String?, latency: Int, error: String?) = await withCheckedContinuation { continuation in
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    if needsInit {
                        ...
                        print("[CP-iOS] Testing llama.cpp backend...")
                        let backendTest = self.bridge.testLlmBackend()
                        print("[CP-iOS] Backend test result: \(backendTest)")
                        ...
                        print("[CP-iOS] Backend test passed!")
                        ...
                        if !FileManager.default.fileExists(atPath: modelPath) {
                            if let bundlePath = Bundle.main.path(forResource: "smollm2", ofType: "gguf") {
                                ...
                                print("[CP-iOS] Using bundled model at: \(modelPath)")
                            }
                        } else {
                            print("[CP-iOS] Using Documents model at: \(modelPath)")
                        }

                        guard FileManager.default.fileExists(atPath: modelPath) else {
                            print("[CP-iOS] No model file found!")
                            continuation.resume(returning: (nil, 0, "No model file. Add smollm2.gguf to Documents."))
                            return
                        }

                        print("[CP-iOS] Initializing LLM with model...")
                        let res = self.bridge.initLlm(modelPath: modelPath)
                        print("[CP-iOS] LLM init result: \(res)")
                    ...
                    // Generate answer
                    print("[CP-iOS] Generating answer for: \(query)")
                    if let res = self.bridge.generate(query: query) {
                        print("[CP-iOS] Generation successful: \(res.latency_ms)ms")
                        continuation.resume(returning: (res.answer, res.latency_ms, nil))
                    } else {
                        let error = self.bridge.getLastError()
                        print("[CP-iOS] Generation failed: \(error)")
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("CP ensures your data is private, deterministic, and verifiable.")
                        .foregroundColor(.secondary)
                }
                ...
                VStack(alignment: .leading, spacing: 16) {
                    Label("Deterministic State", systemImage: "circle.grid.hex.fill")
                        .font(.headline)
                    Text("The State Root represents the mathematical 'thumbprint' of your entire knowledge graph. Because CP is deterministic, identical files will always produce the exact same root across any device.")
                        .font(.subheadline)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    Label("Verifiable Sync", systemImage: "arrow.left.and.right.circle.fill")
                        .font(.headline)
                    Text("When you sync with your Mac, CP only exchanges encrypted 'diffs'. The State Root is used to verify that both devices have perfectly synchronized states without needing to trust the relay server.")
                        .font(.subheadline)
                }
