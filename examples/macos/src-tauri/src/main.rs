...
#[tauri::command]
async fn generate_answer(state: State<'_, AppState>, text: String) -> Result<cp_query::GenerationResult> {
    let qe = {
        let app_lock = state.app.lock().unwrap();
        if let Some(app) = app_lock.as_ref() {
            let graph = app.graph();
            ...
            let intelligence = Box::new(cp_query::OllamaGenerator::new(
                "http://localhost:11434".into(),
                "mistral".into(),
            ));
            QueryEngine::new(graph, embedder).with_intelligence(intelligence)
        } else {
            return Err(cp_core::CPError::NotFound("App not initialized".into()));
        }
...
async fn reset_data(state: State<'_, AppState>) -> Result<String> {
    ...
    *app_lock = None;
    ...
    if state.data_dir.exists() {
        std::fs::remove_dir_all(&state.data_dir).map_err(|e| cp_core::CPError::Io(e))?;
    }
...
async fn get_document_chunks(state: State<'_, AppState>, doc_id: String) -> Result<Vec<UiSearchResult>> {
    ...
    if let Some(app) = app_lock.as_ref() {
        ...
        let id = Uuid::parse_str(&doc_id).map_err(|e: uuid::Error| cp_core::CPError::Parse(e.to_string()))?;
