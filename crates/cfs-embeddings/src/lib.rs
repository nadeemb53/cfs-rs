//! CFS Embeddings - Local embedding generation
//!
//! Uses Candle (pure Rust) to run embedding models locally on CPU.
//! Default model: all-MiniLM-L6-v2 (384 dimensions)
//!
//! Model is downloaded on first use to ~/.cfs/models/
...
impl EmbeddingEngine {
    ...
    pub fn from_path(model_path: &str, config_path: &str, tokenizer_path: &str) -> Result<Self> {
        ...
        let config_str = std::fs::read_to_string(config_path)
            .map_err(|e| CfsError::Embedding(format!("Failed to read config: {}", e)))?;
        let config: Config = serde_json::from_str(&config_str)
            .map_err(|e| CfsError::Embedding(format!("Failed to parse config: {}", e)))?;

        // Load weights
        let vb = unsafe {
            VarBuilder::from_mmaped_safetensors(&[model_path], DTYPE, &device)
                .map_err(|e| CfsError::Embedding(format!("Failed to load weights: {}", e)))?
        };

        // Load model
        let model = BertModel::load(vb, &config)
            .map_err(|e| CfsError::Embedding(format!("Failed to initialize model: {}", e)))?;

        // Load tokenizer
        let mut tokenizer = Tokenizer::from_file(tokenizer_path)
            .map_err(|e| CfsError::Embedding(format!("Failed to load tokenizer: {}", e)))?;
    ...
    fn get_model_dir() -> Result<PathBuf> {
        let home = dirs::home_dir()
            .ok_or_else(|| CfsError::Embedding("Cannot find home directory".into()))?;
        let model_dir = home.join(".cfs").join("models").join("all-MiniLM-L6-v2");
        
        std::fs::create_dir_all(&model_dir)
            .map_err(|e| CfsError::Embedding(format!("Failed to create model dir: {}", e)))?;
    ...
    fn download_file(url: &str, path: &PathBuf) -> Result<()> {
        ...
        let response = ureq::get(url)
            .call()
            .map_err(|e| CfsError::Embedding(format!("Download failed: {}", e)))?;

        let mut body = response.into_body().into_reader();
        let mut file = std::fs::File::create(path)
            .map_err(|e| CfsError::Embedding(format!("Failed to create file: {}", e)))?;
        
        std::io::copy(&mut body, &mut file)
            .map_err(|e| CfsError::Embedding(format!("Failed to write file: {}", e)))?;
    ...
    pub fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        let encodings = self
            ...
            .map_err(|e| CfsError::Embedding(format!("Tokenization failed: {}", e)))?;
        ...
        let input_ids = Tensor::from_vec(input_ids, (batch_size, seq_len), device)
            .map_err(|e| CfsError::Embedding(format!("Failed to create input Tensors: {}", e)))?;
        let token_type_ids = Tensor::from_vec(token_type_ids, (batch_size, seq_len), device)
            .map_err(|e| CfsError::Embedding(format!("Failed to create token type Tensors: {}", e)))?;

        // Forward pass
        let hs = self.model.forward(&input_ids, &token_type_ids, None)
            .map_err(|e| CfsError::Embedding(format!("Inference failed: {}", e)))?;

        // Mean pooling
        let (_b_sz, _seq_len, _hidden_size) = hs.dims3().map_err(|e| CfsError::Embedding(e.to_string()))?;
        
        // Use attention mask for proper pooling
        let mut embeddings = Vec::with_capacity(batch_size);
        for i in 0..batch_size {
            let mask = encodings[i].get_attention_mask();
            let hs_i = hs.get(i).map_err(|e| CfsError::Embedding(e.to_string()))?;
            
            let mut sum = vec![0.0f32; self.dim];
            let mut count = 0.0f32;
            
            let data = hs_i.to_vec2::<f32>().map_err(|e| CfsError::Embedding(e.to_string()))?;
