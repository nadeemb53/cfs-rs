...
//! Per CFS-010: unit vectors are scaled by 32767, quantized with round_ties_even.
//! Per CFS-003 §4.3: canonical operations use integer math.
...
pub struct Embedding {
    ...
    /// Per CFS-001: stored for efficient cosine similarity without recomputation
    pub l2_norm: f32,
}
...
impl Embedding {
    ...
    pub fn new(chunk_id: Uuid, vector_f32: &[f32], model_hash: [u8; 32]) -> Self {
        ...
        // 2. Quantize to i16 (scale by 32767, round_ties_even per CFS-003 §4.3)
    ...
    ///
    /// Per CFS-003 §4.5: all similarity computations use integer math.
...
///
/// Per CFS-003 §4.3: uses round_ties_even for deterministic rounding.
/// Dead-zone: values in (-1e-7, 1e-7) map to 0.
fn quantize_f32_to_i16(val: f32) -> i16 {
    // Dead-zone per CFS-003 §4.3
    if val.abs() < 1e-7 {
        return 0;
    }
