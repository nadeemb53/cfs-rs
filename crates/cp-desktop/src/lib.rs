//! CP Desktop - Tauri desktop application
//!
...
use cp_parser::{Chunker, ChunkConfig};
use tracing::info;
...
impl IngestionWorker {
    ...
    fn process_file(&self, path: &PathBuf) -> Result<()> {
        ...
        *self.status.lock().unwrap() = format!("Parsing {}", filename);
        ...
        // 1. Read metadata
        let metadata = std::fs::metadata(path).map_err(|e| CPError::Io(e))?;
...
impl DesktopApp {
    pub fn new(data_dir: PathBuf) -> Result<Self> {
        std::fs::create_dir_all(&data_dir).map_err(|e| CPError::Io(e))?;
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        for dir in &self.watch_dirs {
            for entry in std::fs::read_dir(dir).map_err(|e| CPError::Io(e))? {
                let entry = entry.map_err(|e| CPError::Io(e))?;
                let path = entry.path();
                if path.is_file() {
                    let filename = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
                    if filename == ".DS_Store" || filename.starts_with('.') {
        ...
        tokio::task::spawn_blocking(move || {
            worker.run(rx);
        }).await.map_err(|e| CPError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
