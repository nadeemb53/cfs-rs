use cp_core::{CognitiveDiff, CPError, Hlc, Result, StateRoot};
use cp_graph::GraphStore;
use cp_relay_client::RelayClient;
use cp_sync::CryptoEngine;
...
impl SyncManager {
    ...
    pub fn new(
        ...
        let crypto = if key_path.exists() {
            let seed = std::fs::read(key_path).map_err(|e| CPError::Io(e))?;
            if seed.len() != 32 {
                return Err(CPError::Crypto("Invalid key file length".into()));
            }
            let mut arr = [0u8; 32];
            ...
            let seed = [1u8; 32];
            std::fs::write(key_path, seed).map_err(|e| CPError::Io(e))?;
    ...
    pub fn record_add_doc(&self, doc: cp_core::Document) {
    ...
    pub fn record_add_chunk(&self, chunk: cp_core::Chunk) {
        let mut diff = self.pending_diff.lock().unwrap();
    ...
    pub fn record_add_embedding(&self, emb: cp_core::Embedding) {
        let mut diff = self.pending_diff.lock().unwrap();
    ...
    pub async fn pull(&self) -> Result<usize> {
        ...
        for root_hex in remote_roots {
            let root_bytes = hex::decode(&root_hex).map_err(|e| CPError::Parse(e.to_string()))?;
