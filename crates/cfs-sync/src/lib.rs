...
use cfs_core::{CognitiveDiff, CfsError, Result};
...
///
/// Per CFS-013: Wraps an encrypted diff with authentication data
...
impl SignedDiff {
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        if expected_device_id != self.sender_device_id {
            return Err(CfsError::Verification("Device ID doesn't match public key".into()));
        }
        ...
        verifying_key
            ...
            .map_err(|_| CfsError::Verification("Invalid SignedDiff signature".into()))
    }
