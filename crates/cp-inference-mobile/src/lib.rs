//! CP Inference Mobile - Local LLM inference for mobile devices
//!
...
impl LocalGenerator {
    ...
    pub fn initialize(&self) -> Result<()> {
        let mut backend_guard = self
            .backend
            .lock()
            .map_err(|_| CPError::Inference("Backend mutex poisoned".into()))?;
        ...
        if !self.model_path.exists() {
            ...
            return Err(CPError::Inference(msg));
        }
        ...
        *backend_guard = Some(backend);
        ...
        let mut model_guard = self
            .model
            .lock()
            .map_err(|_| CPError::Inference("Model mutex poisoned".into()))?;
    ...
    pub fn generate(&self, prompt: &str, max_tokens: u32) -> Result<String> {
        ...
        let backend = backend_guard
            .as_ref()
            .ok_or_else(|| CPError::Inference("Backend not initialized".into()))?;
        ...
        let model = model_guard
            .as_ref()
            .ok_or_else(|| CPError::Inference("Model not initialized".into()))?;
        ...
        let mut ctx = model
            ...
            .map_err(|e| CPError::Inference(format!("Failed to create context: {}", e)))?;

        // Tokenize input
        let tokens = model
            .str_to_token(prompt, llama_cpp_2::model::AddBos::Always)
            .map_err(|e| CPError::Inference(format!("Tokenization failed: {}", e)))?;
        ...
        for chunk in tokens.chunks(n_batch as usize) {
            ...
            for (i, token) in chunk.iter().enumerate() {
                ...
                batch
                    ...
                    .map_err(|e| CPError::Inference(format!("Failed to add token to batch: {}", e)))?;
            }
            ctx.decode(&mut batch)
                .map_err(|e| CPError::Inference(format!("Initial decode failed: {}", e)))?;
            n_past += chunk.len();
        }
        ...
        for _ in 0..max_tokens {
            ...
            batch
                ...
                .map_err(|e| CPError::Inference(format!("Failed to add generated token: {}", e)))?;

            n_cur += 1;

            // Decode
            ctx.decode(&mut batch)
                .map_err(|e| CPError::Inference(format!("Decode step failed: {}", e)))?;
        }
        ...
        for token in &output_tokens {
            match model.token_to_piece_bytes(*token, 32, true, None) {
                ...
                Err(e) => {
                    return Err(CPError::Inference(format!(
...
/// Implement cp_query::IntelligenceEngine for LocalGenerator
#[async_trait::async_trait]
impl cp_query::IntelligenceEngine for LocalGenerator {
    async fn generate(&self, context: &cp_core::AssembledContext, query: &str) -> Result<String> {
        // Format the context using ContextAssembler
        let formatted_context = cp_core::ContextAssembler::format(context);
        ...
        tokio::task::spawn_blocking(move || generator.generate(&prompt_owned, 1024))
            .await
            .map_err(|e| CPError::Inference(format!("Generation task failed: {}", e)))?
    }
}
