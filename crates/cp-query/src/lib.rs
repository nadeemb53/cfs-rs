...
use cp_core::{Chunk, CPError, Result, ContextAssembler, ScoredChunk, AssembledContext};
use glob::Pattern;
use lru::LruCache;
...
impl IntelligenceEngine for OllamaGenerator {
    async fn generate(&self, context: &AssembledContext, query: &str) -> Result<String> {
        ...
        let res = client
            ...
            .send()
            .await
            .map_err(|e| CPError::Embedding(format!("Ollama request failed: {}", e)))?;
        ...
        let answer = json["response"]
            .as_str()
            .ok_or_else(|| CPError::Parse("Invalid Ollama response".into()))?
            .to_string();
...
impl QueryEngine {
    ...
    pub fn get_chunks_for_document(&self, doc_id: Uuid) -> Result<Vec<SearchResult>> {
        ...
        let doc = graph
            .get_document(doc_id)?
            .ok_or_else(|| CPError::Database(format!("Doc {} not found", doc_id)))?;
    ...
    pub async fn generate_answer(&self, query: &str) -> Result<GenerationResult> {
        ...
        let answer = if let Some(ref engine) = self.intelligence {
            ...
            return Err(CPError::NotFound("Intelligence engine not configured".into()));
        };
    ...
    pub fn chat(&self, _query: &str, _history: &[Message]) -> Result<String> {
        ...
        Err(CPError::NotFound("Use generate_answer for Phase 2 initial integration".into()))
    }
}
...
mod tests {
    ...
    async fn test_hybrid_search() {
        ...
        let emb = cp_core::Embedding::new(chunk.id, &vec, embedder.model_hash());
