...
> Purpose: Achieve 100% test coverage across all crates
...
```rust
// Test scenarios for ID generation
#[test]
...
1. **Existing Tests** (`crates/cp-tests/tests/`):
   - `determinism.rs` - 2 tests (clean rebuild, restart)
   - `retrieval.rs` - 1 test (lexical vs vector)
   - `context.rs` - 3 tests (assembly, token budget, formatting)
   - `incremental.rs` - 1 test (performance baseline - no assertions)
   - `sync_audit.rs` - 1 test (simple convergence)

2. **Critical Missing Test Areas**:
   - All unit tests for cryptographic functions
   - HNSW index persistence and consistency
...
1. **P0 - Critical** (Required for production):
   - ID generation test vectors
   - Merkle tree proofs
   - Crypto encryption/decryption
   - State root computation
   - CognitiveDiff application

2. **P1 - High** (Required for reliability):
   - Transaction atomicity
   - Cascade delete behavior
   - Cache invalidation
   - Error handling

3. **P2 - Medium** (Required for completeness):
